...
/* USER CODE BEGIN PD */
#define SINGLE_TIMER_OPT // do NOT undefine
...
/* USER CODE BEGIN GV */
VTIMER_HandleType timerHandle = {0};
...
uint32_t MX_APPE_Init(void *p_param)
{
  ...
  SMRSubGConfig a = MRSUBG_DEFAULT_CFG();
  MRSubG_PcktBasicFields b = MRSUBG_DEFAULT_FRAME_CFG();
  mrsubg_init(&a);
  mrsubg_frame_init(&b);

  /* Start the VTimer  */
  timerHandle.callback = CPU_WKUP_IRQHandler;
...
void MX_APPE_Process(void)
{
  ...
  HAL_MRSUBG_TIMER_StopVirtualTimer(&timerHandle);
  HAL_MRSUBG_TIMER_StartVirtualTimerMs(&timerHandle, 125);
...
void CPU_WKUP_IRQHandler(void* ptr) {
